...
import { ArrowLeft, ArrowRight, Check, ExternalLink, Eye, Info, LayoutTemplate, Loader2, Mailbox, Plus, Search, Sparkles, X } from "lucide-react";
    ...
    {Array.from({ length: total }, (_, i) => i + 1).map((s) => (
      <React.Fragment key={s}>
        <div className={cn("flex items-center gap-2 cursor-pointer group transition-all", s > current && "opacity-60 hover:opacity-100")} onClick={() => onStepClick(s)}>
          ...
          <span className={cn("text-sm font-semibold hidden sm:block", s === current ? "text-primary" : "text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-300")}>{STEP_LABELS[s - 1]}</span>
        </div>
  ...
  return (
    <div className="mx-auto p-4 sm:p-8 space-y-6">
      ...
      <div className="bg-white dark:bg-(--card-color) rounded-lg border border-slate-200/60 dark:border-(--card-border-color) p-6 shadow-sm">
        <StepIndicator current={step} total={totalSteps} onStepClick={handleStepClick} />
      </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 text-[10px] font-bold text-primary hover:text-primary/80 transition-colors uppercase tracking-widest px-1"
                  >
