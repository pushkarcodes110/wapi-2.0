...
const MessageTemplates = ({ wabaId, statusFilter: statusFilterProp, onToggleSidebar }: MessageTemplatesProps) => {
  ...
  return (
    <div className="p-4 pt-0! sm:p-6 space-y-8 h-screen flex flex-col min-w-0 overflow-hidden">
      <CommonHeader title={t("templates_page_title")} description={t("templates_page_description")} featureKey="template_bots_used" onRefresh={handleRefresh} onSync={handleSync} onSyncStatus={handleSyncStatus} syncStatusPermission="create.template" isLoading={isLoading} isSyncingStatus={isSyncingStatus} onAddClick={() => router.push(`${ROUTES.MessageTemplates}/${wabaId}/create`)} addLabel={t("create_new_template")} addPermission="create.template" onSearch={setSearchQuery} searchTerm={searchQuery} searchPlaceholder="Search templates..." rightContent={WabaDisplay} onToggleSidebar={onToggleSidebar}>
