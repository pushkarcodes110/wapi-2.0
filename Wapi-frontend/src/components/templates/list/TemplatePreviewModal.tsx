    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-[min(95vw,400px)] bg-transparent border-none shadow-none p-0 flex flex-col items-center justify-center overflow-visible">
        ...
        <div className="w-full relative">
          ...
          <FormLivePreview templateType={category === "AUTHENTICATION" ? "none" : template.header?.format?.toLowerCase() || "none"} headerText={category === "AUTHENTICATION" ? "" : template.header?.text || ""} messageBody={template.message_body || ""} variables_example={variables} footerText={template.footer_text || ""} buttons={template.buttons || []} headerFile={null} mediaUrl={template.header?.media_url || undefined} marketingType={marketingType} offerText={template.offer_text} productCards={productCards} mediaCards={mediaCards} authData={authPreviewData} />
        </div>
      </DialogContent>
    </Dialog>
  );
