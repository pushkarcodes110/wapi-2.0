        ...
        <div className="faq-image p-6 sm:p-10 md:p-16 lg:p-20 px-[calc(20px+(145-20)*((100vw-320px)/(1920-320)))] flex flex-col rounded-[32px] sm:rounded-[48px] lg:rounded-[64px] lg:flex-row gap-10 items-start overflow-hidden relative">
          <div className="grid grid-cols-1 min-[1200px]:grid-cols-3 gap-[calc(0px+(40-0)*((100vw-320px)/(1920-320)))] w-full">
                ...
                <svg width="490" height="448" viewBox="0 0 498 456" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <defs>
                    ...
                    <linearGradient id="paint1_linear_132_472" x1="249" y1="3.99998" x2="81.0583" y2="337.554" gradientUnits="userSpaceOnUse">
                      ...
                      <stop offset="1" stopColor="var(--page-body-bg)" />
                    </linearGradient>
                  </defs>
                </svg>
              </div>
            ...
            <div className="flex-1 w-full space-y-4 z-10 min-[1200px]:col-span-2 max-h-202.5 overflow-auto custom-scrollbar">
              {faqs.map((item, index) => (
                <div key={index} className={`transition-all duration-300 faq-box overflow-hidden rounded-[10px] ${activeIndex === index ? "bg-primary border-transparent" : "bg-landing-body-dark border-[var(--background)20] hover:border-landing-success/40"}`}>
                  <button onClick={() => toggleAccordion(index)} className={`w-full flex items-center justify-between p-[calc(8px+(20-8)*((100vw-320px)/(1920-320)))] md:p-6 text-left transition-colors duration-300 rounded-[10px] ${activeIndex === index ? "bg-landing-bg-light rounded-tl-[10px] rounded-tr-[10px] rounded-bl-none rounded-br-none" : "bg-landing-body-dark"} `}>
                    <div className="flex items-center gap-4">
                      ...
                      <span className={`font-semibold text-[18px] transition-colors ${activeIndex === index ? "text-landing-text-dark" : "text-white"}`}>{item.title}</span>
                    </div>
                  </button>
