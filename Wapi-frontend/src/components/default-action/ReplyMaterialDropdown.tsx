...
const ReplyMaterialDropdown: React.FC<ReplyMaterialDropdownProps> = ({ value, onChange, placeholder = "Select material...", wabaId, disabled = false }) => {
  ...
  const allGroups = useMemo<GroupedOptions[]>(() => {
    ...
    const groups: GroupedOptions[] = [
      ...
      { label: "Form Flow", ref_type: "ReplyMaterial_flow", items: filterName(flows).map((i) => ({ _id: i._id, name: i.name, type: "flow", ref_type: "ReplyMaterial" })) },
      { label: "Appointment", ref_type: "appointment", items: filterName(appointments.map(a => ({ ...a, name: a.name || "" }))).map((i) => ({ _id: i._id, name: i.name, type: "appointment", ref_type: "appointment" })) },
    ];
    ...
    <div ref={dropdownRef} className="relative">
      ...
      {open && (
        <div className="absolute z-50 top-full mt-1.5 w-full min-w-64 bg-white dark:bg-(--card-color) border border-slate-200 dark:border-(--card-border-color) rounded-xl shadow-2xl overflow-hidden">
          ...
          <button type="button" onClick={() => window.open(ROUTES.ReplyMaterials, "_blank")} className="w-full flex items-center gap-1.5 px-3 py-2.5 text-sm font-bold text-primary hover:bg-primary/5 transition-colors border-b border-slate-100 dark:border-(--card-border-color)">
            <Plus size={14} />
            Add new +
          </button>
