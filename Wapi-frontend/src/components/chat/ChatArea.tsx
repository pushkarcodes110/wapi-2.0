...
const ChatArea: React.FC<ChatAreaProps> = ({ contactId, phoneNumberId, contactName, contactNumber, contactAvatar, isModal = false }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const localMedia = selectedMedia.filter((m: any) => m.localFile);
      const remoteMedia = selectedMedia.filter((m: any) => !m.localFile);

      if (localMedia.length > 0) {
        // If there are multiple local files or a mix, upload local ones first to get URLs
        // because the send API only handles one file_url upload at a time.
        let finalMediaUrls = remoteMedia.map((m) => m.fileUrl);

        if (localMedia.length === 1 && remoteMedia.length === 0) {
          // Optimization: If there's exactly one local file and no other media, use the send API's direct upload
