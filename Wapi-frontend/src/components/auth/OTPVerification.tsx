        ...
        <div className="grid lg:grid-cols-2 gap-0 items-center">
          <div className="hidden lg:flex flex-col justify-center p-12 bg-linear-to-br from-emerald-600 to-teal-700 rounded-l-3xl relative overflow-hidden min-h-150">
            ...
            <div className="relative z-10 space-y-8">
              ...
              <div>
                <h2 className="text-4xl text-white mb-4 leading-tight">{t("verify_email_address_title")}</h2>
                <p className="text-emerald-100 text-lg">{t("verification_code_sent_desc")}</p>
              </div>
              ...
              <div className="space-y-4">
                <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg p-5">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-white font-semibold mb-1">{t("check_inbox")}</p>
                      <p className="text-emerald-100 text-sm">
                        {t("sent_code_to")} <span className="font-semibold">{email}</span>
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg p-5">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-white font-semibold mb-1">{t("valid_for_10_mins")}</p>
                      <p className="text-emerald-100 text-sm">{t("code_expires_desc")}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-4">
                <p className="text-emerald-200 text-sm font-medium mb-2">💡 {t("didnt_receive_it_tip")}</p>
                <ul className="text-emerald-100 text-sm space-y-1">
                  <li>• {t("check_spam_bullet")}</li>
                  <li>• {t("ensure_email_correct", { email })}</li>
                  <li>• {t("wait_and_request_bullet")}</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white lg:rounded-r-3xl shadow-2xl p-8 lg:p-12 min-h-150 flex flex-col justify-center relative">
            ...
            <div className="relative z-10 max-w-md mx-auto w-full">
              ...
              <div className="mb-8">
                <h2 className="text-3xl text-slate-900 font-bold mb-3">{t("enter_verification_code")}</h2>
                <p className="text-slate-600">
                  {t("code_sent_to")} <span className="font-semibold text-slate-900">{email}</span>
                </p>
              </div>

              <form onSubmit={formik.handleSubmit}>
                <div className="mb-8">
                  <label className="block text-sm font-medium text-slate-700 mb-4">{t("6_digit_code_label")}</label>
