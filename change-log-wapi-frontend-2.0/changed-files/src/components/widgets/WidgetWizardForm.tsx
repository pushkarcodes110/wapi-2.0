          ...
          {STEPS.map((step, idx) => {
            ...
            return (
              <React.Fragment key={step.id}>
                <button onClick={() => handleStepClick(idx)} className={cn("flex items-center gap-2 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-sm font-medium transition-all duration-300 shrink-0", isActive ? "bg-primary text-white shadow-lg shadow-primary/30" : isDone ? "bg-primary/10 text-primary hover:bg-primary/20" : "text-slate-400 dark:text-gray-500 hover:text-slate-600 hover:bg-slate-50 group dark:hover:bg-(--table-hover)")}>
