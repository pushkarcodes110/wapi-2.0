...
const TemplateMessage: React.FC<TemplateMessageProps> = ({ message, isWindowExpired }) => {
  ...
  return (
    <BaseMessage message={message} isWindowExpired={isWindowExpired}>
      <div className={`flex flex-col -mx-3 sm:-mx-4 -my-1.5 mb-1.5 min-w-[200px] overflow-hidden ${isOutgoing ? "rounded-lg rounded-te-none" : "rounded-lg rounded-ts-none"}`}>
        {isLimitedTimeOffer && (
        ...
        {!isSpecial && template.header && (
          <>
            ...
            {template.header.format === "image" && message.fileUrl && (
              <div className="overflow-hidden -mx-4 -mt-1.5 mb-1 aspect-video relative">
                <Images src={message.fileUrl} alt="Template Header" fill className="object-cover" unoptimized />
              </div>
            )}
        ...
        <div className="ps-4 pe-3 py-2">
          <p className="whitespace-pre-wrap wrap-break-word leading-normal text-[13.5px] dark:text-white">{template.message_body}</p>
        </div>

        {template.footer_text && (
          <div className="ps-4 pe-3 pb-2 pt-0">
            <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">{template.footer_text}</p>
          </div>
        )}
