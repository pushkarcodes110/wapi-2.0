...
const CampaignWizard = () => {
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (carouselHasLocalFiles) {
          ...
          const cleanCarouselCardsData = (payload.carousel_cards_data as any[])?.map((card: any) => {
          ...
          const sanitizedPayload = { ...payload, carousel_cards_data: cleanCarouselCardsData };

          // Append all keys except complex ones
          Object.entries(sanitizedPayload).forEach(([key, value]) => {
            if (key === "carousel_cards_data" || key === "carousel_products" || key === "variables_mapping" || key === "specific_contacts" || key === "tag_ids") {
  ...
  const validateCurrentStep = (stepIdx: number) => {
    ...
    if (stepIdx === configStepIndex && (!formik.values.waba_id || !formik.values.template_id)) return "Please select WABA and Template";
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isActive ? "bg-white dark:bg-(--card-color) shadow-lg shadow-slate-200/50 dark:shadow-none ring-1 ring-slate-200/50 dark:ring-(--card-border-color)" : "opacity-60 hover:opacity-100"
                )}
              ...
              >
                ...
                <div className="min-w-0">
                  <h3 className={cn("font-black text-sm uppercase tracking-wider line-clamp-1 transition-colors", isActive ? "text-primary dark:text-white" : "text-slate-500 dark:text-gray-400 group-hover:text-slate-700 dark:group-hover:text-slate-200")}>{step.title}</h3>
